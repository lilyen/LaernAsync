...
namespace D025.Console專案的工作進度與取消
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var progress = new Progress<int>((s) => { Console.WriteLine($"下載進度為 {s}%"); });
        ...
        static async Task 下載檔案內容(string page, CancellationToken token, IProgress<int> progress)
        {
            ...
            await Task.Run(async () =>
            {
                using (HttpClient client = new HttpClient())
                {
                    using (HttpResponseMessage response =
                        await client.GetAsync(page, HttpCompletionOption.ResponseHeadersRead, token))
                    {
            ...
            #endregion 使用 HttpClient 與 TAP 模式來設計
